...
ros::Publisher pub_odometry,ekf_pub_odometry, pub_latest_odometry;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    ekf_pub_odometry = n.advertise<nav_msgs::Odometry>("ekf_odometry", 1000);
                ...
                Eigen::Matrix3d loop_correct_r)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        nav_msgs::Odometry odometry;
        odometry.header = header;
        odometry.header.frame_id = "base_footprint";
        odometry.child_frame_id = "vo";//world
        ...
        odometry.pose.covariance={10,0,0,0,0,0,
                                  0,1e-2,0,0,0,0,
                                  0,0,1e-2,0,0,0,
                                  0,0,0,1e-4,0,0,
                                  0,0,0,0,1e-4,0,
        ...
        odometry.twist.covariance={1e-6,0,0,0,0,0,
                                   0,1e-6,0,0,0,0,
                                   0,0,1e-6,0,0,0,
                                   0,0,0,1e-4,0,0,
                                   0,0,0,0,1e-4,0,
 ...
 /*       odometry.child_frame_id = "wheelodom";

        odometry.pose.pose.position.x = correct_t.x();//+(rand() % (3-0))+ 0;
        odometry.pose.pose.position.y = correct_t.y();//(rand() % (3-0))+ 0;
        odometry.pose.pose.position.z = correct_t.z();//+(rand() % (3-0))+ 0;
        ekf_pub_odometry.publish(odometry);*/
        pose_stamped.pose = odometry.pose.pose;
        odometry.child_frame_id = "wheelodom";
        float tt=gaussrand();
        //odometry.pose.pose.position.y = correct_t.y()+gaussrand();
        odometry.pose.pose.position.x = correct_t.x()+tt;
        odometry.pose.pose.position.y = correct_t.y()+tt;
        odometry.pose.pose.position.z = correct_t.z()+tt;
        ekf_pub_odometry.publish(odometry);
                   ...
                   Eigen::Matrix3d loop_correct_r)
{
    ...
    //robot frame
   /* transform.setOrigin(tf::Vector3(0.02,0.002,0.02));
    q.setW(1.0);
    q.setX(0.0);
    q.setY(0.0);
    q.setZ(0.0);
    transform.setRotation(q);
    br.sendTransform(tf::StampedTransform(transform, header.stamp, "robot", "body"));*/
    ...
    q.setZ(0.0);
    transform.setRotation(q);
    br.sendTransform(tf::StampedTransform(transform, header.stamp, "base_footprint", "vo"));
    // IMU-BASE frame
    transform.setOrigin(tf::Vector3(0.0,0.0,0.0));
    q.setW(1.0);
    q.setX(0.0);
    ...
    q.setZ(0.0);
    transform.setRotation(q);
    br.sendTransform(tf::StampedTransform(transform, header.stamp, "base_footprint", "wheelodom"));
}
double gaussrand()
{
